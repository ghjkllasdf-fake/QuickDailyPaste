...
### Quick Start
1. Download `QuickDailyPaste.exe` from [Releases](../../releases) and double-click to run (lives in the system tray)
...
| File | Description |
|------|-------------|
| `QuickDailyPaste.exe` | Main executable (double-click to run) |
...
### Distribution
Just share `QuickDailyPaste.exe` — that's all the recipient needs. Works on Windows 7+ (no .NET install required since .NET Framework 4 is system-bundled).
...
### 快速开始
1. 从 [Releases](../../releases) 下载 `QuickDailyPaste.exe`，双击运行（系统托盘常驻）
...
### 分发
直接把 `QuickDailyPaste.exe` 发给对方即可，无需其他文件。Windows 7+ 系统自带 .NET Framework 4，开箱即用。
